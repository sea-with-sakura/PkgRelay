...
function renderPools(sources, stats) {
  ...
  $("sources").innerHTML = poolDefinitions.map((pool) => {
    ...
    return `<article class="source clickable" data-pool="${pool.name}" role="button" tabindex="0"><span class="tag">${pool.name}</span><h3>${pool.title} 缓存池</h3><p>${usage.blob_count} 个对象 · ${formatBytes(usage.total_bytes)} · ${usage.total_hits} 次命中</p><p class="source-action">查看内容 →</p></article>`;
  }).join("");
...
async function loadSourceArtifacts() {
  ...
  try {
    ...
    $("detail-count").textContent = query ? `匹配 ${result.total} 个物理对象，显示 ${shown} 个` : `共 ${result.total} 个物理对象，显示 ${shown} 个`;
    $("detail-artifacts").innerHTML = result.artifacts.length ? result.artifacts.map((item) => {
      return `<tr><td class="path">${escapeHtml(item.path)}</td><td>${formatBytes(item.size)}</td><td>${item.hit_count}</td><td>${new Date(item.fetched_at * 1000).toLocaleString()}</td><td class="upstream">${escapeHtml(item.upstream_url)}</td></tr>`;
