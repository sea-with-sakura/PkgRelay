...
客户端会在使用 `conda`、`pip` 或 `pip3` 时检查网关版本。若网关已发布新版客户端，会自动下载、校验、替换客户端脚本并登记新版本，然后继续原包操作；只有自动升级失败时才阻止包操作。

网关强制要求客户端版本一致。版本缺失或落后的客户端访问 Conda/PyPI 路由时会收到 HTTP 426；正常情况下本地包装器会先自动升级，因网络或文件权限导致升级失败时，重新执行接入命令并选择 **Install** 即可恢复。
...
> **不支持 `python -m pip` / `python3 -m pip`。** 这类命令不会经过 PkgRelay 的 Bash 包装器，因而不保证使用中枢缓存、客户端自动升级或版本门禁。请统一使用 `pip` 或 `pip3`。
...
| 新 pip cache | 不保留 | 受支持的 `pip` / `pip3` 命令自动追加 `--no-cache-dir`。 |
